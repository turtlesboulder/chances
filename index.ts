...
button?.addEventListener("click", ()=>{
    ...
    let trials = document.querySelector("#trials") as HTMLInputElement
    let chance = document.querySelector("#chance") as HTMLInputElement
    let needed = document.querySelector("#needed") as HTMLInputElement
    let n:number
    let c:number
    let s:number
    try{
        n = trials.value as any as number
        c = chance.value as any as number
        s = needed.value as any as number
...
function binomialProbability(n:number, k:number, c:number):number{
    const coefficient = binomialCoefficient(n, k)
    return coefficient * (c**k) * ((1-c)**(n-k))
}
function totalProbability(n:number, c:number, s:number):number{
    if (n > MAX || s > n) {
        return -1;
    }
...
function normalApproximation(n:number, c:number, s:number):number{
    // unfinished
    let mean = n*c
...
function makeLineString(x1:number, y1:number, x2:number, y2: number, style:string="stroke:black; stroke-width:2"):string{
    let line = `<line x1="${x1}" x2="${x2}" y1="${y1}" y2="${y2}" style="${style}" />`
   return line
}
function makeTextString(x:number, y:number, text:string):string{
    let textString = `<text x="${x}" y="${y}" fill="black">${text}</text>`
    return textString
}
function makeGridLines(numLines:number):string{
...
function annotateGridLines(s:number, n:number):string{
...
function makeLineStrings(s:number, n:number, c:number):string{
    let lines = ""
    let xScale = WIDTH / (n-s)
...
function makeLineGraph(s:number, n:number, c:number){
